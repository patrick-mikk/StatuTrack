...
def main() -> int:
    ...
    xml_bytes = load_xml(source)
    print(f"  read {len(xml_bytes):,} bytes")
    ...
    if source.startswith(("http://", "https://")):
        ...
        parsed.source_url = f"file://{Path(source).resolve()}"
    parsed.parsed_at = dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    # Detect document type from the root element: <Statute> or <Regulation>.
    # Anything else falls back to statute parsing as a best-effort default.
    root_tag = etree.QName(root).localname
    if root_tag == "Regulation":
        parsed.document_type = "regulation"
    else:
        parsed.document_type = "statute"
